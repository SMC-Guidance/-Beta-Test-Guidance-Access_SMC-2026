...
## Patched build changelog (security fixes 1-5)

This build hardens the items flagged in review:

1. **Session token storage.** The frontend now stores the login token in
   `sessionStorage` (cleared when the tab/browser closes) instead of
   `localStorage`, matching the model described above. Trusted-device memory
   (for skipping the 2FA email) stays in `localStorage` by design.
2. **Cryptographic randomness.** Salts, session/share tokens, and 2FA codes no
   longer depend on `Math.random()`. A new `secureRandomHex()` derives values
   from multiple platform UUIDs through a keyed HMAC; `randomToken()` and the
   2FA code generator (`secureCode6()`) use it.
3. **Tighter read scope.** `listForms` and `getFormResponses` now require staff
   (admin / co-admin) instead of any authenticated user.
4. **Exact identity matching.** Record, evaluation, and incident visibility use
   `fieldMatchesAny()` (exact, delimiter-aware matching) instead of substring
   `indexOf`, closing the name-collision leak. NOTE: the `designate` /
   Assigned To / Checked By fields must now contain exact names/usernames
   (comma-separated for several people).
5. **Server-side share sanitizing.** Public share pages are passed through
   `sanitizeShareHtml()` before storage, stripping scripts, dangerous tags,
   inline event handlers, and script-y URL schemes.

Still open (see "Honest limits" above): no login rate-limiting beyond the
attempt lockout, iterated HMAC rather than bcrypt/argon2, and the underlying
Google Sheet stores PII in plaintext. Regex HTML sanitizing is a mitigation,
